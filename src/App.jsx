...
import GttStatusCard from "./components/GttStatusCard";
...
function MetricRow({ icon: Icon, label, value, unit }) {
  return (
    <div className="flex items-baseline py-2">
      {/* Fixed label column keeps values aligned. */}
    ...
    <Card title="Traffic" icon={Car}>
      <div className="mx-auto w-full max-w-[420px]">
        <div className="grid grid-cols-3 gap-4 items-end">
          ...
          {/* Heavy and jam counters aligned on the right. */}
...
export default function App() {
    ...
    (async () => {
      try {
        ...
        const [snapS, jamsS, histS, redditS] = await Promise.allSettled([
          ...
          // Demo: read the latest 50 posts from api.reddit_torino_posts.
      ...
      } catch (e) { if (import.meta.env.DEV) console.error(e); setErr("Failed to load demo data."); }
      finally {
        setLoading(false);
  ...
  return (
    <div className="app-bg app-viewport text-slate-900 dark:text-slate-100">
      {loading && (
        <div className="fixed top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 via-indigo-500 to-fuchsia-500 animate-pulse z-[9999]" />
      )}
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {loading && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-indigo-50 text-indigo-600 border border-indigo-100 dark:bg-indigo-950/40 dark:text-indigo-300 dark:border-indigo-900/60 animate-pulse">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <RefreshCcw size={16} className={isRefreshing ? "animate-spin" : ""} /><span className="text-xs">{isRefreshing ? "Refreshing..." : "Refresh"}</span>
            </button>
          </div>
        </div>
        ...
        {/* GTT Live Transit (wider section below the main top dashboard grid) */}
        <GttStatusCard API_BASE={API_BASE} tick={tick} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 lg:grid-rows-[500px_auto] gap-6 items-start">
          ...
          <div className="lg:col-span-2 lg:row-start-2 grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card title="Air Pollutants (latest)" icon={Activity}>
              ...
              <div className="relative h-56 rounded-lg overflow-hidden">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    {/* Bars without repeated numeric labels */}
                    <Bar
                      dataKey="value"
                      isAnimationActive
                      animationBegin={120}
                      ...
                      shape={(props) => {
                        // Custom RoundedBar hook can be used here if needed:
                        // return <RoundedBar {...props} />;
                        // Or use a simple custom shape:
                    ...
                    {/* Soft gradients for each pollutant */}
                    <defs>
                      {pollutants.map((p) => {
              ...
              {/* Compact legend without repeated numbers */}
              <div className="mt-3 flex items-center gap-4 text-[11px]">
                ...
                {/* Optional summary showing the maximum pollutant */}
                {(() => {
             ...
             {/* --- Temperature · Polished --- */}
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    {/* Soft background and horizontal grid lines */}
                    <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.22} />

                    {/* Comfort band: 18–24 °C; adjust values if needed. */}
                    <ReferenceArea
                      y1={18}
                      y2={24}
                      ifOverflow="extendDomain"
                    ...
                    {/* Gradient used for fill and stroke. */}
                    <defs>
                    ...
                    {/* Area and top line; points are shown through the tooltip. */}
                    <Area
                      type="monotone"
                      dataKey="temp"
