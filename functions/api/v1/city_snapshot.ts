...
export const onRequest: PagesFunction<Env> = async (ctx) => {
  ...
  try {
    // This demo reads the latest snapshot without applying a city filter.
    ...
    if (!raw) return okJSON({}, { "Cache-Control": "s-maxage=30, stale-while-revalidate=60" });
    ...
    return errJSON(500, "snapshot failed");
  }
};
